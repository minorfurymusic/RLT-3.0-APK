...
export default function AIAssistant() {
  ...
  const handleSend = async (customText?: string) => {
    ...
    try {
      ...
      // 2. Complex reasoning query requiring Gemini API
      const ai = getGoogleGenAI();
      const safeProfile = anonymizeProfile(profile);

      const healthContext = JSON.stringify({
        perfil: safeProfile,
        resumoHoje: {
          aguaBebidaMl: totalWaterMl,
          metaAguaMl: Math.round(getDailyWaterTarget() * 1000),
          caloriasConsumidas: totalCaloriesEaten,
          metaCalorias: Math.round(getDailyCalorieTarget()),
          refeicoesRegistradas: todayMeals.map(m => `${m.description} (${m.calories}kcal)`),
        },
        historicoExamesEConsultas: historyRecords.slice(0, 10).map((r: any) => ({
          categoria: r.category,
          nome: r.examName || r.conditionName || r.doctorName || r.title,
          data: r.date,
          status: r.status
        })),
        agendamentosCalendario: events.slice(0, 5).map(e => ({
          titulo: e.title,
          data: e.date,
          horario: e.time,
          tipo: e.type
        })),
        metasAtivas: goals.filter(g => g.selected).map(g => g.title),
        idioma: appLanguage,
      }, null, 2);
      ...
      for (const att of currentAttachments) {
        if (att.data.includes(';base64,')) {
          const [header, base64Data] = att.data.split(';base64,');
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: `Você é o Motor Central do Real Life Track (RLT ENGINE V3 — POLÍTICA DE EXECUÇÃO OBRIGATÓRIA & GOVERNANÇA TRANSACIONAL).

PIPELINE DE GOVERNANÇA OBRIGATÓRIO (FASES 0 A 9):
Antes de responder ou executar qualquer comando, o Engine executa as Fases 0 a 7 do Pipeline de Governança (Carregamento, Auditoria, Integridade, Autoavaliação, Validação de 11 Regras, Validação de 17 Domínios, Teste de Obrigação) e gera Prova de Utilização para cada regra.

REGRAS ABSOLUTAS:
1. Você NÃO é um chatbot. Você é o Motor Central do Real Life Track.
2. Sua única responsabilidade é transformar linguagem natural em alterações e consultas consistentes na Base Central.
3. Você NÃO responde antes de verificar/atualizar e realizar read-back na Base Central.
4. Você NÃO pode inventar dados, ignorar informações, resumir registros ou misturar domínios.
5. Cada informação pertence a um domínio específico.
6. Nunca use frases inteiras do usuário como nomes de registros; sempre normalize com precisão.
7. Responda em tom profissional, altamente objetivo, direto e estruturado.
Idioma: ${appLanguage === 'pt-BR' ? 'Português do Brasil' : 'English'}.

Base Central RLT (Dados Reais do Usuário):
${healthContext}`,
        }
      });

      const aiContent = response.text || t('ai.error');
